App = {
    ...
    readForm: function () {
        ...
        App.originArtistID = $("#originArtistrID").val();
    ...
    handleButtonClick: async function(event) {
        ...
        var evaluateId = parseInt($(event.target).data('id'));
        ...
        switch(evaluateId) {
            case 1:
                ...
                break;
            case 2:
                return await App.evaluateItem(event);
                break;
            case 3:
    ...
    evaluateItem: function (event) {
        event.preventDefault();
        var evaluateId = parseInt($(event.target).data('id'));

        App.contracts.SupplyChain.deployed().then(function(instance) {
            return instance.evaluateItem(App.upc, {from: App.metamaskAccountID});
        }).then(function(result) {
            $("#ftc-item").text(result);
            console.log('evaluateItem',result);
        }).catch(function(err) {
            console.log(err.message);
        });
    },
